...
libs <- c("forecast","expsmooth","TSA","knitr","gglot2", "readxl","dplyr","plyr",
          "dynlm","Hmisc","car","AER", "magrittr","tseries", "dLagM","stats", "x12")
libraries(libs)
...
getAutoModels <- function(tsobject) {
  ...
  for(i in 1:nrow(xg)){
    fit.auto <- ets(tsobject,model="ZZZ", damped = xg[i,2], ic=toString(xg[i,1]),opt.crit = "amse")
...
getSesModels <- function(tsobject, forecasting_period) {
  ...
  rm(i,initials,dampeds,exponentials,xg)
  return (lst)
}
...
getHoltWintersModels <- function(tsobject, forecasting_period){
  ...
  rm(i,seasonals,dampeds,exponentials,xg)
  return (lst)
}
...
getEtsModels <- function(id,tsobject){
  e <- c("A","M","N")
  t <- c("A","M","N")
  s <- c("A","M","N")
  dampeds <- c(TRUE, FALSE)
  xg <- expand.grid(e,t,s,dampeds)
  model_summ_df = data.frame()
  ts_id = NA
  ...
  for(i in 1:nrow(xg)){
    ...
    tryCatch({
      if(mt == "NNN") {
        fit.ets <- ets(tsobject, damped = xg[i,4], ic=c("aicc", "aic", "bic"))
        ts_id = id
        model_df = data.frame(cbind(ts_id,fit.ets$aic,fit.ets$bic,
                                   fit.ets$aicc,accuracy(fit.ets)[6],fit.ets$method))
      } else if(xg[i,3] != "N") {
        if(frequency(tsobject) != 1) {
          fit.ets <- ets(tsobject, model=mt, damped=xg[i,4], ic=c("aicc", "aic", "bic"))
          ts_id = id
          model_df = data.frame(cbind(ts_id,fit.ets$aic,fit.ets$bic,
                                   fit.ets$aicc,accuracy(fit.ets)[6],fit.ets$method))
        }
   ...
   model_summ_df = rbind.fill(model_summ_df,model_df)
  }
  rm(k,mt,fit.ets,i,e,t,s,dampeds,xg)
...
```{r}
getAllModelsMase <- function(id,tsobject, forecasting_period) {
#  maselst <- data.frame()
...
#  maselst <- getEtsModels(tsobject)
  return (getEtsModels(id,tsobject))
}
```
...
for(i in c(1,2,3)) {
  ...
  #sampling to make execution faster - should be deleted later
  df = df[sample(nrow(df),5),]
...
models_data = lapply(all_objects,
                     function(element){
                        t = getAllModelsMase(element@id,element@ts_training, 
                                            element@forecast_period)
                        return(t)
                      })
models_data = as.data.frame(do.call(rbind,models_data))
models_data = models_data[rowSums(is.na(models_data)) != ncol(models_data),]
colnames(models_data) = c("TS_ID","AIC","BIC","AICC","MASE","METHOD")
models_data$AIC = as.numeric(as.character(models_data$AIC))
models_data$BIC = as.numeric(as.character(models_data$BIC))
models_data$AICC = as.numeric(as.character(models_data$AICC))
models_data$MASE = as.numeric(as.character(models_data$MASE))
...
sorted <- models_data %>% 
          group_by(TS_ID) %>%
          mutate(my_ranks = order(AICC, AIC,BIC, decreasing=TRUE))
...
fit.auto <- ets(all_objects[[1]]@ts_training,model="ZZZ", damped = xg[1,2], ic=toString(xg[1,1])
                ,opt.crit = "mse"
                )
accuracy(fit.auto)
fit.auto$method
k = ifelse(xg[1,2],paste(toString(xg[1,1]),"damped", sep='_'),toString(xg[1,1]))
  ...
  dampeds <- c(TRUE, FALSE)
  xg <- expand.grid(e,t,s,dampeds)
  mt <- toString(paste(xg[15,1],xg[15,2],xg[15,3],sep = ""))

fit.auto <- ets(all_objects[[6]]@ts_training,model="AAN", damped = TRUE, ic=c("aicc", "aic", "bic"))

test = data.frame(cbind(fit.auto$aic,fit.auto$bic,fit.auto$aicc,fit.auto$mase,fit.auto$method,fit.auto$series ))
print(t)
